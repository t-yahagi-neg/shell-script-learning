...
# プロンプト定義
export PS1="${CYAN}Ubuntu $(parse_git_branch)${CYAN}\$ ${RESET}"

# スクリプトのパス
SCRIPT_DIR="$HOME/dev/scripts"
LOGFILE="$HOME/dev/scripts/test.log"

script() {
	local script_dir="$HOME/dev/scripts/shell-recipe"
	...
	{
		echo -e "${GREEN}===== 実行開始: ${timestamp_start} =====${RESET}"
	...
	if [[ $exit_code -eq -0 ]]; then
		...
		echo -e "${RED}スクリプトは異常終了しました(コード：$exit_code)：${timestamp_end}${RESET}" | tee -a "$log_file"
	fi
...
# export PATH="$HOME/dev/shell-script-learning:$PATH"
